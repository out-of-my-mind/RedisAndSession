  ...
  public sealed class OdbcSessionStateStore : SessionStateStoreProviderBase
  {
      ...
      bool newItem)                                           
    {
      ...
      if (newItem)
      {
        ...
        cmd = new OdbcCommand("INSERT INTO Sessions " + " (SessionId, ApplicationName, Created, Expires, " + "  LockDate, LockId, Timeout, Locked, SessionItems, Flags) " +
          " Values(?, ?, ?, ?, ?, ? , ?, ?, ?, ?)", conn);
      ...
      out SessionStateActions actionFlags)
    {
      ...
      try
      {
        ...
        if (lockRecord)
        {
          //修改会话，条件：同一区间（ApplicationName）、未锁定（Locked）、同一ID（SessionId）、未过期（Expires）
